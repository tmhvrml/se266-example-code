...
//  5) Return results if needed.
    ...
    public function getProfilePicURL($user) 
    {
        ...
        $stmt = $userTable->prepare("SELECT userPic FROM users WHERE userName = :user");

        // Bind query parameters to method parameter values
        $stmt->bindValue(':user', $user);    
        
         // Execute query and check to see if rows were returned 
         // If so, the user was successfully fetched
        $fetchSucessful = ($stmt->execute() && $stmt->rowCount() > 0);
        if ($fetchSucessful)
        {
            $results = $stmt->fetch(PDO::FETCH_ASSOC); 
        }
